...
This pipeline:
1. Breaks the video into 7 temporal segments (one per column)
...
def extract_column_region(frame, column_idx, num_columns, width):
    """Extract a column region from a frame."""
...
class SegmentedColumnsPipeline(BasePipeline):
    ...
    def __init__(self, *args, num_segments: int = 7, **kwargs):
        ...
        Args:
            num_segments: Number of temporal segments to break video into (default 7, one per column)
