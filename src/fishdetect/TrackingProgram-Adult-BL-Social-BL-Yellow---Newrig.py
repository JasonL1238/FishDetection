...
Namebg = 'input/Clutch1_20250804_122715.mp4'
...
#Number of rows and columns of wells - MODIFIED FOR 4x7 LAYOUT
nRow = 4
nCol = 7
...
#Define pixels - ADJUSTED FOR 4 ROWS
glassrow1=42
glassrow2=136  # Adjusted for 4 rows: 544/4 = 136
...
# MODIFIED: Limit to first 30 frames for testing
nFrames = min(nFrames, 30)
...
#Define some variables that are used to track the points along the back
    ...
    # Create overlay image for shape outlining and centroid labeling
    overlay = np.zeros((im_height, im_width, 3), dtype=np.uint8)
    overlay[:,:,0] = img  # Red channel - original image
    overlay[:,:,1] = img  # Green channel - original image  
    overlay[:,:,2] = img  # Blue channel - original image
...
#find the centroid of        
        ...
        # MODIFIED: Updated glass row boundaries for 4 rows
        if cent[0] < glassrow1:
            continue  
        ...
        if cent[0] > glassrow4 and cent[0]<glassrow5:
            continue
...
#find the centroid of        
        ...
        for col in range(1, nCol+1):
            if cent[1] < col*nPixelx/nCol:
                fishcol = col
                # print("x" + str(cent[0])+"y" + str(cent[1])+"test" + str(col*nPixelx/nCol))
                break
    ...
    return im_tmp, y_cent, x_cent, overlay
...
for index, img in enumerate(tqdm(video)):
    ...
    #won't affect old one
            ...
            timg, y_cent, x_cent, overlay = tracking(bg, img, ind)
            ...
            if SaveTrackingIm:
                ...
                # ADDED: Save overlay image with shapes and centroids
                cv2.imwrite(inDir + 'Results/TrackedVideos/' + Name + '/trackedimages/' + Name + '_overlay_' + str(index) +'.png', overlay)        
...
#designate which fish are mut/het/wt - UPDATED FOR 28 FISH
muts = np.array([])
